    ...
    try:
        async with session.get(DEXSCREENER_SEARCH_URL, timeout=10) as response:
            ...
            for pair in data["pairs"]:
                ...
                txns_h1 = pair.get("txns", {}).get("h1", {}).get("buys", 0) + pair.get(
                    "txns", {}
                ).get("h1", {}).get("sells", 0)
        ...
        print(f"Network/API Error in dexscreener_scanner: {type(e).__name__}")
        ...
        print(f"Data Processing Error in dexscreener_scanner: {type(e).__name__} - {e}")
        return []  # Return empty list on error
